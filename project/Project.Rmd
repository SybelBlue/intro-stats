...
marital_trump <- raw_survey_data %>%
  ...
  filter(is.known(trump_approval), is.known(marital), is.finite(age)) %>%
  mutate(approves_trump = endsWith(trump_approval, "Approve")) %>%
  mutate(is_married = marital == "Married")
```

``` {r}
marital_model <- glm(approves_trump ~ age * is_married, marital_trump, family = "binomial")
summary(marital_model)$coefficients

marital_predictions <- marital_trump %>% 
  mutate(probability = marital_model$fitted.values) %>% 
  mutate(odds = probability / (1-probability)) %>% 
  mutate(log_odds = log(odds))

ggplot(marital_predictions, aes(x = age, y = probability, color = is_married)) + 
